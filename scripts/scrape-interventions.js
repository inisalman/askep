...
async function scrapeInterventions() {
  ...
  for (const diag of diagnosisUrls) {
    try {
      ...
      const interventions = await page.evaluate(() => {
        ...
        // Find all h3 elements that contain intervention codes (I.xxxxx)
        const h3Elements = document.querySelectorAll('h3');
        
        h3Elements.forEach(h3 => {
          const text = h3.textContent.trim();
          // Match pattern like "Manajemen Nyeri (I.08238)"
          const match = text.match(/^(.+?)\s*\(I\.(\d+)\)$/);
      ...
      if (interventions.length > 0) {
        interventionsData[diag.code] = interventions;
        console.log(`  ✓ Found ${interventions.length} interventions`);
        successCount++;
      } else {
        interventionsData[diag.code] = [];
  ...
  fs.writeFileSync(
    '/root/askep-gh/src/data/interventions-detail.json',
    JSON.stringify(interventionsData, null, 2)
  );
  
  console.log(`\nDone! Success: ${successCount}, Failed: ${failCount}`);
  console.log('Saved to /root/askep-gh/src/data/interventions-detail.json');
}

scrapeInterventions().catch(console.error);
